      ...
      <div className="min-h-screen bg-gray-50/50 pb-12">
        <header className="bg-white border-b border-gray-100 sticky top-0 z-30">
          <div className="max-w-3xl mx-auto px-4 h-16 flex items-center justify-between">
            <Link href="/dashboard/recipient" className="text-gray-500 hover:text-[#4CAF50] flex items-center">
        ...
        <main className="max-w-3xl mx-auto px-4 pt-8 space-y-4">
          {loading ? (
            ...
            history.map((h) => (
              <Card key={h.matchId} className="border-0 shadow-sm bg-white hover:shadow-md transition-shadow">
                <CardContent className="p-5">
                  <div className="flex items-start space-x-4">
                    {/* Thumbnail */}
                    <div className="w-16 h-16 rounded-xl overflow-hidden bg-gray-100 flex-shrink-0 border border-gray-100 shadow-sm">
                      {h.item?.images?.[0] ? (
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between">
                        ...
                        <CheckCircle className="w-5 h-5 text-[#4CAF50] flex-shrink-0 ml-2" />
                      </div>

                      <div className="flex items-center flex-wrap gap-2 mt-2">
                        {/* Donor */}
                        <div className="flex items-center space-x-1.5">
                          <div className="w-5 h-5 rounded-full bg-[#4CAF50]/10 overflow-hidden flex-shrink-0 flex items-center justify-center text-[10px] font-bold text-[#2E7D32]">
                            {h.donor?.avatarUrl ? (
